...
Broadcaster.getPackages = (id, type, result) => {
  if (type == 'archive') {
    sql.query(
      `SELECT broadcasters.package_archive_ids as package_ids FROM videos LEFT JOIN broadcasters ON videos.broadcaster_id=broadcasters.id WHERE videos.hlsurl='${id}' OR videos.poster='${id}'`,
      async(err, res) => {
        if (err) {
          console.log("error: ", err);
          result(null, err);
        ...
        if (res.length) {
          sql.getConnection((error, connection) => {
            ...
            connection.query(`SELECT * FROM packages WHERE id IN (${res[0].package_ids}) AND expiry_date > NOW() ORDER BY price`, (error, datas) => {
              if (error) {
                throw error;
              }
              connection.release();
