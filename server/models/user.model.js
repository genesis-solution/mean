        ...
        if (res && res.length > 0) {
          sql.getConnection((err, conn) => {
            ...
            conn.query(`SELECT * FROM admission_history WHERE user_id='${sp_user_id}' AND hlsurl='${id}' AND created_at >= DATE_SUB(NOW(), INTERVAL 2 DAY)`, (error, adminssion_history) => {
              ...
              if (adminssion_history && adminssion_history.length > 0) {
                result(null, true);
              } else {
                if (res[0].uses > 0) {
...
User.purchaseLiveState = (sp_user_id, v_id, type, result) => {
  sql.query(
    `SELECT purchase.id, purchase.broadcaster_id, purchase.broadcast_id, broadcasts.name, purchase.uses FROM purchase LEFT JOIN broadcasts ON purchase.broadcast_id=broadcasts.id WHERE purchase.sp_user_id='${sp_user_id}' AND purchase.broadcast_id='${v_id}' AND purchase.live AND purchase.created_at >= DATE_SUB(NOW(), INTERVAL 1 DAY) LIMIT 1`,
        (err, res) => {
        if (err) {
          console.log("update balance by id error: ", err);
          result(err, null);
            ...
            connection.query(`UPDATE purchase SET uses=uses+1 WHERE id=${res[0].id}`, (error, datas) => {
              ...
              sql.getConnection((error, conn) => {
                ...
                conn.query(`INSERT INTO admission_history(user_id, broadcaster_id, broadcast_id, purchase_id, archive, video_name, remain_uses, created_at) 
                    VALUES ('${sp_user_id}', ${res[0].broadcaster_id}, ${res[0].broadcast_id}, ${res[0].id}, 0, '${res[0].name}', ${res[0].uses}, NOW())`,
                 (error, dt) => {
                  if (error) {
                    throw error;
                  }
